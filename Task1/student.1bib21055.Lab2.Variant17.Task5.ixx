export module student_1bib21055_Lab2_Variant17_Task5;
import <cmath>;
...
double RBPO::Lab2::Variant17::Task5::funcF1(double x)
{
	return sqrt(pow(3 * x + 2, 2) - 24 * x) / (3 * sqrt(x) + 2 / sqrt(x));
}
double RBPO::Lab2::Variant17::Task5::funcF2(double x)
{
	return (x >= 1.1) ? (9 - x) : (sin(3 * x) / (pow(x, 4) + 1));
}
double RBPO::Lab2::Variant17::Task5::funcA(int i)
{
	return (pow(-1, i) * (1 - ((double(i * i + 1) / double(i * i + 3)))));
}
double RBPO::Lab2::Variant17::Task5::funcF3(int n)
{
	double f3 = 0;
	for (int i = 1; i <= n; i++)
	{
...
double RBPO::Lab2::Variant17::Task5::funcF4(double e)
{
